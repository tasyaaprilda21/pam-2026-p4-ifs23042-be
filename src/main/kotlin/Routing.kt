...
fun Application.configureRouting() {
    ...
    val bagService: BagService by inject()
    ...
    routing {
        get("/") {
            call.respondText("API telah berjalan. Dibuat oleh Tasya Aprilda Marbun.")
        }
        ...
        // Route Bags <-- TAMBAHAN BARU
        route("/bags") {
            get {
                bagService.getAllBags(call)
            }
            post {
                bagService.createBag(call)
            }
            get("/{id}") {
                bagService.getBagById(call)
            }
            put("/{id}") {
                bagService.updateBag(call)
            }
            delete("/{id}") {
                bagService.deleteBag(call)
            }
            get("/{id}/image") {
                bagService.getBagImage(call)
            }
        }
    }
}